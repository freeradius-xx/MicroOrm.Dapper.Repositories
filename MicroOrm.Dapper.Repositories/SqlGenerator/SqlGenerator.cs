...
namespace MicroOrm.Dapper.Repositories.SqlGenerator
{
        ...
        where TEntity : class
    {
        ...
        public virtual SqlQuery GetUpdate(TEntity entity)
        {
            ...
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(string.Format("UPDATE " + TableName + " SET "+ string.Join(", ", properties.Select(p => p.ColumnName + " = @" + p.Name)) + " WHERE "+ string.Join(" AND ", KeySqlProperties.Select(p => p.ColumnName + " = @" + p.Name))));
